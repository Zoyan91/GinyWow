...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post('/api/optimize', upload.single('thumbnail'), async (req, res) => {
    try {
      ...
      // Validate file type (jpg/png only as per prompt)
      ...
      // Enhanced thumbnail processing based on prompt requirements:
      ...
      let optimizedBuffer = await sharp(req.file.buffer)
        // Auto-center crop for better thumbnail dimensions if needed
        .resize(1280, 720, { 
          fit: 'cover', 
          position: 'center',
          withoutEnlargement: true // Don't enlarge smaller images
        })
        // Enhance brightness, contrast, saturation for better visual appeal
        .modulate({
          brightness: 1.15, // +15% brightness
          saturation: 1.2,  // +20% saturation for more vibrant colors
          hue: 0
        })
        // Enhance sharpness for crisp thumbnails
        .sharpen(1.5, 1, 2)
        // Apply contrast enhancement
        .linear(1.1, -12.8) // +10% contrast
