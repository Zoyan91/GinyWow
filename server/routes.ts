...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Resize Image Tool
  app.post('/api/image-tools/resize', imageToolsUpload.single('image'), async (req, res) => {
    try {
      if (!req.file) {
        return res.status(400).json({ error: 'No image file provided' });
      }
      ...
      if (maintainAspectRatio === 'true' && width && height) {
      ...
      res.json({
        ...
        downloadName: `resized_${req.file.originalname}`
      });
    } catch (error) {
  ...
  // Compress Image Tool  
  app.post('/api/image-tools/compress', imageToolsUpload.single('image'), async (req, res) => {
    try {
      if (!req.file) {
        return res.status(400).json({ error: 'No image file provided' });
      }
      ...
      // Preserve original format while compressing
      ...
      res.json({
        ...
        processedImage: `data:${mimeType};base64,${base64Result}`,
        downloadName: `compressed_${req.file.originalname}`
      });
    } catch (error) {
  ...
  // Convert Format Tool
  app.post('/api/image-tools/convert', imageToolsUpload.single('image'), async (req, res) => {
    try {
      if (!req.file) {
        return res.status(400).json({ error: 'No image file provided' });
      }
      ...
      switch (format) {
        ...
        case 'jpeg':
        case 'jpg':
          processedBuffer = await sharp(req.file.buffer).jpeg().toBuffer();
          mimeType = 'image/jpeg';
          extension = 'jpg';
          break;
        case 'webp':
          processedBuffer = await sharp(req.file.buffer).webp().toBuffer();
          mimeType = 'image/webp';
          extension = 'webp';
          break;
        default:
  ...
  // OCR Tool
  app.post('/api/image-tools/ocr', imageToolsUpload.single('image'), async (req, res) => {
    try {
      if (!req.file) {
        return res.status(400).json({ error: 'No image file provided' });
      }

      // Note: This is a placeholder - Tesseract.js should be implemented on frontend
      // for better performance and to avoid server load
      res.json({
        success: true,
        extractedText: 'OCR functionality will be implemented on the frontend using Tesseract.js for better performance.',
        confidence: 0,
        message: 'Please implement OCR on the frontend'
      });
    } catch (error) {
